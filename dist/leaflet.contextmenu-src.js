...
L.Map.ContextMenu = L.Handler.extend({
        ...
        addHooks : function () {
            L.DomEvent.on(document, L.Browser.touch ? "touchstart" : "mousedown", this._onMouseDown, this).on(document, "keydown", this._onKeyDown, this);
            this._map.on({
        ...
        insertSet : function (options, id) {
            id = id !== undefined ? id : this._sets.length;
            var set = this._createSet(options, id);
            this._sets.push(set);
            return set
        },
        ...
        setHidden : function (item, hidden) {
            ...
            } else if (item && L.DomUtil.hasClass(item, separatorCls)) {
                if (hidden) {
        ...
        _changeState : function (state) {
            ...
            i,
            l;
            if (state !== this._state) for (i = 0, l = this._sets.length; i < l; i++) {
                set = this._sets[i];
                if (set.state === state || (set.name === state && set.state !== this._state)) {
                    //console.log("change state: " + set.name);
        ...
        _createSet : function (options, id) {
            name = options.name !== undefined ? options.name : 'set_' + id;
            return {
                id : id,
                name : options.name,
                state : options.state
